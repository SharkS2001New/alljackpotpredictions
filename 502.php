...
<title>502 — Bad Gateway | AllJackpotPredictions</title>
<meta name="description" content="AllJackpotPredictions received an invalid response from an upstream server. Our team has been notified. Please try again in a few minutes.">
...
<style>
/* ═══════════════════════════════════════════
   502 BAD GATEWAY — page-scoped styles
═══════════════════════════════════════════ */
...
[data-theme="dark"] .err-gd-box { color: var(--navy2); }
.err-gd-box.you   { border-color: var(--navy); background: rgba(11,61,46,.06); }
[data-theme="dark"] .err-gd-box.you { background: rgba(45,143,106,.1); }
.err-gd-box.proxy { border-color: var(--amber); background: rgba(180,83,9,.06); }
.err-gd-box.upstream {
  border-color: rgba(201,41,42,.4); background: rgba(201,41,42,.06);
  color: var(--red, #c9292a);
}
.err-gd-label {
...
.err-gd-arrow {
  font-family: var(--fmono); font-size: .8rem; color: var(--border2);
  padding: 0 8px; flex-shrink: 0; margin-top: -14px;
}
.err-gd-arrow.broken { color: var(--red, #c9292a); }
.err-gd-error {
  font-family: var(--fmono); font-size: .52rem; font-weight: 700;
  letter-spacing: .06em; text-transform: uppercase;
  color: var(--red, #c9292a); margin-top: 2px;
}
...
.err-retry-btn:hover { opacity: .85; }
.err-retry-countdown {
  font-family: var(--fmono); font-size: .7rem; font-weight: 700;
  color: var(--muted); white-space: nowrap;
}
.err-retry-countdown span { color: var(--amber); }
...
.err-status-strip {
  ...
  background: rgba(180,83,9,.06); border: 1px solid rgba(180,83,9,.22);
  margin-top: 4px;
}
.err-status-dot {
...
<nav class="breadcrumb-wrap" aria-label="Breadcrumb">
  <div class="breadcrumb">
    ...
    <span class="bc-cur">502 Bad Gateway</span>
  </div>
</nav>
...
<div class="err-page">
  <div class="err-inner">
    ...
    <div>
      ...
      <span class="err-code">502</span>
      <span class="err-label">Bad Gateway</span>

      <p class="err-desc">
        AllJackpotPredictions' server received an <strong>invalid response from an upstream server</strong>
        while trying to fulfil your request. This is a temporary infrastructure issue —
        not a problem with your browser or connection.
      </p>

      <div class="err-callout">
        <strong>This is on our end.</strong> A 502 means the gateway or proxy server sitting
        in front of AllJackpotPredictions got a bad response from the application server behind it.
        Our team has been notified. This type of error typically resolves within a few minutes.
      </div>

      <!-- Gateway diagram -->
      <div class="err-gateway-diagram">
        <div class="err-gd-node">
          <div class="err-gd-box you">You</div>
          <div class="err-gd-label">Browser</div>
        </div>
        <div class="err-gd-arrow">→</div>
        <div class="err-gd-node">
          <div class="err-gd-box proxy">Gateway</div>
          <div class="err-gd-label">CDN / Proxy</div>
        </div>
        <div class="err-gd-arrow broken">⇢</div>
        <div class="err-gd-node">
          <div class="err-gd-box upstream">✕ Upstream</div>
          <div class="err-gd-label">App Server</div>
          <div class="err-gd-error">Bad Response</div>
        </div>
      </div>
      ...
      <div class="err-meta-strip">
        ...
        <div class="err-meta-item">
          <div class="err-meta-val" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo $uri; ?></div>
      ...
      <div class="err-causes">
        <div class="err-cause-item">
          ...
          <span>The application server may be <strong>restarting</strong> after a deployment or crash. These gaps are usually brief — try again in 30–60 seconds.</span>
        </div>
        <div class="err-cause-item">
          <span class="err-cause-icon">🌐</span>
          <span>The upstream server may be <strong>overloaded</strong> and unable to return a valid response in time. Traffic spikes around kick-off time can occasionally cause this.</span>
        </div>
        <div class="err-cause-item">
          <span class="err-cause-icon">🔧</span>
          <span>A <strong>deployment or maintenance window</strong> may be in progress. If this error appears site-wide, check our social channels for a status update.</span>
        </div>
        <div class="err-cause-item">
          <span class="err-cause-icon">✉️</span>
          <span>If this error persists for more than a few minutes, <a href="/contact">contact us</a> with the <strong>Error ID</strong> above so we can investigate.</span>
        </div>
      </div>
    ...
    <div class="err-nav">
      ...
      <div class="err-status-strip">
        ...
        <span class="err-status-text">HTTP 502 · Bad Gateway</span>
      </div>
    </div>
